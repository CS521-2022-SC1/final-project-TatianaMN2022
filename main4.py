# Home Price Calculator Version 1.0
...
class analyze_values:
  ...
  def do_regression(self,df_X_test):

    # Store passed in test data in class data
    ...
    self.df_X_train = self.modifieddf[self.independent_variables]

    self.df_Y_train = self.modifieddf[['SalePrice']]
  ...
  def print_report(self):
    print("Predicted Price: \n", "${:,.2f}".format(float(self.eSalePrice[0])))
...
Garage = input("Enter garage size (number of cars):")
dict = {'GrLivArea': [Sq_feet], 'BedroomAbvGr': [Bedrooms],'YearBuilt':[Yr_Built],'GarageCars':[Garage] }
df_test = pd.DataFrame(dict)
myAV.do_regression(df_test)
myAV.print_report()